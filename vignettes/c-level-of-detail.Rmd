...
library(choroplethr)
data(choroplethr)
state_choropleth(df_pop_state, title="2012 State Population Estimates, Continuous Scale", num_buckets=1) 
```
...
```{r}
state_choropleth(df_pop_state, title="2012 State Population Estimates, 9 Buckets", num_buckets=9) 
```
...
```{r}
state_choropleth(df_pop_state, title="2012 State Population Estimates, 2 Buckets", num_buckets=2) 
```
...
state_choropleth(df_pop_state, title="States with Populations Above or Below 1 Million")
```
