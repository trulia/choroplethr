...
choroplethr uses object oriented programming to support users creating choropleths with their maps. If you are interested in this functionality then I recommend you read two articles before proceeding:
...
A key component of object oriented programming is recognizing common functionality and placing it in a base class. The base class in version 2.0 of the choroplethr package is called Choropleth and has five main functions:

1. `initialize(map.df, user.df)`. This is the constructor. map.df is a data.frame representation of a shapefile and needs a column called "region". user.df is the user's data and needs columns called "region" and "value".
...
In this case, the default behavior of the base class is exactly what we want. We just needed to initialize a child class with the map we want.

Note that using the base Choropleth gives you a lot of functionality for free:
