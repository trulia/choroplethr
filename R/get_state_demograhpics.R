...
{  
  ...
  df_race = data.frame(region                   = as.character(tolower(acs::geography(race.data)$NAME)), #as.character(acs::geography(race.data)$zipcodetabulationarea), 
  ...
  # per capita income 
  df_income = choroplethr::get_acs_data("B19301", "state", endyear=2013, span=5)[[1]]  
  colnames(df_income)[[2]] = "per_capita_income"
  
  # median rent
  df_rent = get_acs_data("B25058", "state", endyear=2013, span=5)[[1]]  
  colnames(df_rent)[[2]] = "median_rent"
  
  # median age
  df_age = get_acs_data("B01002", "state", endyear=2013, span=5, column_idx=1)[[1]]  
