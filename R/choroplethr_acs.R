...
#' choroplethr_acs("B19301", "county", scale=1, states=c("NY", "NJ", "CT"))
#'
#' # median income, all zip codes
#' choroplethr_acs("B19301", "zip") } 
#' @importFrom acs acs.fetch geography estimate geo.make
choroplethr_acs = function(tableId, map, endyear=2011, span=5, scale=7, zoom=NULL)
{
  stopifnot(map %in% c("state", "county", "zip"))
  stopifnot(scale > 0 && scale < 10)
  ...
  if (map=="state") {
    state_choropleth(acs.df, title, "", scale, zoom)
  } else if (map=="county") {
    county_choropleth(acs.df, title, "", scale, zoom)
  } else if (map=="zip") {
    zip_map(acs.df, title, "", scale, zoom)
  }
}
